...
namespace Mcucpp
{
	namespace IO
	{
		...
		namespace Private
		{
			template<class GpioRegs, int ID>
			...
			public:
				...
				{
					GpioRegs()->BSRR = (uint16_t)(value & 0xFFFF);
				}
	...
	#define MAKE_PORT(GPIO_REGS, className, ID) \
	   namespace Private{\
...
//	MAKE_PORT(GPIOJ, Portj, 'J')
//	MAKE_PORT(GPIOK, Portk, 'K')
